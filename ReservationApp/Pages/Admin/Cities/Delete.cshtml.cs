...
namespace ReservationApp.Pages.Admin.Cities
{
    ...
    public class DeleteModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int? id)
        {
            ...
            RestaurantsUsing = await _context.Restaurants.Where(r => r.CityId == id).OrderBy(r => r.Name).ToListAsync();
        ...
        public async Task<IActionResult> OnPostAsync(int? id)
        {
            ...
            var restaurantsUsing = await _context.Restaurants.Where(r => r.CityId == id).OrderBy(r => r.Name).ToListAsync();
            if (restaurantsUsing.Count > 0)
            {
                City = city;
                RestaurantsUsing = restaurantsUsing;
                RestaurantsUsingIt = restaurantsUsing.Count;
                errorMessage = $"This city is used by {restaurantsUsing.Count} restaurant(s) and cannot be deleted.";
                return Page();
            }
